...
def render_modo_demo():
    ...
    <div style="background:rgba(108,134,196,0.06);
        ...
        <div style="font-size:0.95rem;color:#E4EDE9;font-weight:600;
                    margin-bottom:6px;">
            Roteiro do pitch - {len(ROTEIRO_DEMO)} etapas em ~{duracao_total}s</div>
        <div style="font-size:0.82rem;color:#8FA39B;line-height:1.7;">
    ...
    st.markdown(f"""
    <div style="background:linear-gradient(135deg,rgba(108,134,196,0.08),
                                                  rgba(99,168,124,0.04));
                border:1px solid rgba(108,134,196,0.3);border-radius:14px;
                padding:1.5rem;margin:1.5rem 0;">
        <div style="font-size:0.7rem;color:#8FA39B;text-transform:uppercase;
                    letter-spacing:0.12em;margin-bottom:8px;">
            Etapa {etapa['etapa']} - duracao sugerida: {etapa['duracao']}s</div>
        <div style="font-size:1.05rem;color:#E4EDE9;line-height:1.7;
                    margin-bottom:14px;font-weight:500;">
            "{etapa['fala']}"</div>
        <div style="font-size:0.85rem;color:#6C86C4;padding-top:14px;
                    border-top:1px solid rgba(108,134,196,0.2);">
            <strong>Destaque tecnico:</strong> {etapa['destaque']}</div>
        <div style="margin-top:14px;font-size:0.72rem;color:#5A6B65;">
            Pagina sugerida: <code style="color:#63A87C;">{etapa['page_id']}</code>
    ...
    for i, e in enumerate(ROTEIRO_DEMO):
        ...
        cor   = "#6C86C4" if ativo else "rgba(224,236,231,0.07)"
        bg    = "#141F1C" if ativo else "#0E1614"
        st.markdown(f"""
        <div style="background:{bg};border:1px solid {cor};
                    border-left:3px solid {cor};border-radius:8px;
                    padding:10px 16px;margin-bottom:6px;
                    {'box-shadow: 0 0 0 2px rgba(108,134,196,0.15);' if ativo else ''}">
            <div style="display:flex;justify-content:space-between;
                        align-items:center;">
                <div>
                    <span style="font-size:0.78rem;color:#E4EDE9;
                                 font-weight:{600 if ativo else 400};">
                        {e['etapa']}</span>
                    <span style="font-size:0.72rem;color:#8FA39B;
                                 margin-left:8px;">- {e['fala'][:80]}...</span>
                </div>
                <span style="font-size:0.7rem;color:#5A6B65;">
                    {e['duracao']}s</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
...
def render_sobre():
    ...
    <div style="text-align:center;padding:2rem 0;
                border-bottom:1px solid rgba(224,236,231,0.07);
                margin-bottom:2rem;">
        <div style="font-size:0.7rem;color:#8FA39B;text-transform:uppercase;
                    letter-spacing:0.15em;margin-bottom:8px;">
            FIAP Challenge 2026 - Sprint 1</div>
        <h1 style="font-size:3rem;font-weight:800;color:#E4EDE9;
                   letter-spacing:-0.02em;line-height:1;margin:0;">
            Sompo Predict <span style="color:#6C86C4;">v15</span></h1>
        <p style="color:#8FA39B;margin-top:1rem;font-size:1.05rem;
    ...
    for col, (icone, valor, desc) in zip(
        ...
        col.markdown(f"""
        <div style="background:#0E1614;border:1px solid rgba(224,75,69,0.2);
                    border-top:3px solid #E04B45;border-radius:12px;
                    padding:1.25rem;text-align:center;height:160px;">
            <div style="font-size:1.8rem;font-weight:800;color:#E04B45;">
                {icone}</div>
            <div style="font-size:0.78rem;color:#E4EDE9;font-weight:600;
    ...
    for col, (titulo, desc, cor) in zip(
        ...
        [
            ("Edge IoT",       "ESP32 + sensores no campo",            "#9B79C0"),
            ("ML Cloud",       "XGBoost + LSTM via AWS",               "#E04B45"),
            ("XAI Dashboard",  "SHAP por fator de risco",              "#6C86C4"),
            ("Auditoria",      "Hash chaining SHA-256",                 "#63A87C"),
        ]):
        col.markdown(f"""
        <div style="background:#0E1614;border:1px solid {cor}33;
            ...
            <div style="font-size:0.78rem;color:#8FA39B;line-height:1.5;">
                {desc}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    membros = [
        ("R", "Rafael",    "Scrum Master, XAI, Pitch",            "#6C86C4",
         "US02 Governanca, US07 Subscricao, vidoe Trello"),
        ("C", "Charles",   "Backend, Seguranca, Integracao",       "#63A87C",
         "US01 Risco, US03 Cliente, Cybersec, App principal"),
        ("A", "Anthony",   "IoT, ESP32, Edge Computing",            "#9B79C0",
         "US04 Operador, ESP32+DHT22, Wokwi"),
        ("Gu", "Guilherme", "Data Science, Estatistica",             "#DDA53C",
         "US06 Tecnico, US08 Sinistros, base SUSEP, ML"),
        ("Gv", "Gustavo",   "Cloud AWS, Flask API",                   "#E04B45",
    ...
    for col, (init, nome, papel, cor, entregas) in zip(cols, membros):
        ...
        <div style="background:#0E1614;border:1px solid rgba(224,236,231,0.07);
            ...
            <div style="font-weight:700;font-size:1rem;color:#E4EDE9;
                        margin-bottom:4px;">{nome}</div>
            <div style="font-size:0.7rem;color:{cor};margin-bottom:8px;">
                {papel}</div>
            <div style="font-size:0.65rem;color:#8FA39B;line-height:1.5;
                        padding-top:8px;
    ...
    stack = [
        ("Python 3.13",     "Linguagem principal",                "#6C86C4"),
        ("Streamlit",       "Frontend interativo",                 "#E04B45"),
        ("scikit-learn",    "Modelos de ML supervisionado",        "#DDA53C"),
        ("XGBoost / LSTM",  "Predicao + series temporais",         "#9B79C0"),
        ("PyJWT",           "Autenticacao com tokens",             "#63A87C"),
        ("Argon2id",        "Hash de senhas (OWASP 2024)",         "#63A87C"),
        ("Fernet (AES)",    "Criptografia em repouso",             "#63A87C"),
        ("AWS EC2",         "Backend FastAPI em producao",         "#E04B45"),
        ("ESP32",           "Sensor IoT com DHT22 + GPS",          "#9B79C0"),
        ("Plotly",          "Graficos interativos",                "#6C86C4"),
    ]
    cols = st.columns(5)
    for i, (tech, desc, cor) in enumerate(stack):
        ...
        <div style="background:#141F1C;border:1px solid {cor}33;
            ...
            <div style="font-weight:700;color:{cor};font-size:0.82rem;">
                {tech}</div>
            <div style="font-size:0.7rem;color:#8FA39B;margin-top:2px;">
                {desc}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div style="text-align:center;padding:1.5rem 0;
                border-top:1px solid rgba(224,236,231,0.07);
                font-size:0.75rem;color:#5A6B65;">

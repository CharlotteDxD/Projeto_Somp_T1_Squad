...
@sintetico(
    "propostas de subscrição fictícias, com nomes de fazenda inventados",
    motivo="np.random gera cliente, valor, score e histórico — nenhuma "
           "proposta corresponde a uma apólice real da base",
    substituir_por="fila real de subscrição integrada ao cadastro de "
                   "apólices (core_integracao.carregar_apolices)",
)
...
@sintetico(
    ...
    motivo="np.random distribui magnitude e sinal de cada fator a partir "
           "do score — não há shap.TreeExplainer sobre um modelo treinado",
    substituir_por="shap.TreeExplainer sobre o modelo real "
                   "(ver core_integracao.carregar_modelo_risco)",
)
...
def render():
    ...
    selo_multiplo([
        origem_por_funcao(__name__, "_propostas_mock"),
        origem_por_funcao(__name__, "_shap_proposta"),
    ], titulo="Fila de subscrição — dados de demonstração")
    ...
    _header(
        ...
        "Recomendação assistida e memória de cálculo do prêmio",
    )
            ...
            <div style="background:#0E1614;border:1px solid {cor}33;
                ...
                <div style="display:flex;justify-content:space-between;
                    ...
                    <div style="flex:1;">
                        <div style="display:flex;gap:10px;align-items:center;
                            ...
                            <span style="font-size:0.65rem;color:{cor};
                                ...
                                {cat.replace('_', ' ')}</span>
                            <span style="font-weight:700;color:#E4EDE9;
                                         font-size:0.95rem;">{row['ID']}</span>
                            <span style="font-size:0.78rem;color:#8FA39B;">
                                {row['Cliente']}</span>
                        </div>
                        <div style="font-size:0.78rem;color:#8FA39B;
                                    line-height:1.6;">
                    ...
                    <div style="text-align:right;min-width:140px;">
                        ...
                        <div style="font-size:0.65rem;color:#8FA39B;
                                    margin-top:2px;">SCORE / 100</div>
                        <div style="margin-top:8px;padding-top:8px;
                                    border-top:1px solid rgba(224,236,231,0.07);
                                    font-size:0.72rem;color:#8FA39B;">
                            Premio: <strong style="color:#E4EDE9;">
    ...
    with tab2:
        ...
        with col_a:
            st.markdown(f"""
            <div style="background:#141F1C;border:1px solid {cor}44;
                ...
                <div style="font-size:0.85rem;color:#8FA39B;margin-top:4px;">
                    / 100</div>
                ...
                <div style="margin-top:1rem;padding-top:1rem;
                            border-top:1px solid rgba(224,236,231,0.07);
                            font-size:0.78rem;color:#8FA39B;
                            text-align:left;line-height:1.7;">
                    <div><strong style="color:#E4EDE9;">{prop['Cliente']}</strong></div>
        ...
        with col_b:
            ...
            for fator, valor in sorted(contribs.items(), key=lambda x: -x[1]):
                ...
                cor_b = ("#E04B45" if valor > 12 else
                         "#DDA53C" if valor > 6 else
                         "#6C86C4" if "Base" in fator else
                         "#63A87C")
                st.markdown(f"""
                <div style="margin-bottom:10px;">
                    <div style="display:flex;justify-content:space-between;
                                font-size:0.85rem;margin-bottom:4px;">
                        <span style="color:#E4EDE9;">{fator}</span>
                        <span style="color:{cor_b};font-weight:700;">
                            +{valor:.1f}</span>
                    </div>
                    <div style="background:#080D0C;height:6px;border-radius:3px;">
    ...
    with tab3:
        ...
        with col_sim2:
            ...
            st.markdown(f"""
            <div style="background:#141F1C;border:1px solid {cor}40;
                ...
                <div style="margin-top:1rem;padding-top:1rem;
                            border-top:1px solid rgba(224,236,231,0.07);
                            font-size:0.85rem;color:#E4EDE9;line-height:2;">
        ...
        if PLOTLY_OK:
            ...
            with col_v1:
                fig1 = px.histogram(
                    ...
                    color_discrete_sequence=["#6C86C4"],
                )
    ...
    <div style="background:rgba(108,134,196,0.04);
        ...
        <strong style="color:#E4EDE9;">Status de integracao US07:</strong><br>
        <span style="color:#63A87C;">x</span> Pagina criada e funcional ·
        <span style="color:#63A87C;">x</span> Decomposicao SHAP-like ·
        <span style="color:#63A87C;">x</span> Decisao auditada via SHA-256 ·
        <span style="color:#63A87C;">x</span> Calculo de premio dinamico ·
        <span style="color:#DDA53C;">~</span> Aguardando XGBoost real (Guilherme/Rafael)
    </div>
    """, unsafe_allow_html=True)

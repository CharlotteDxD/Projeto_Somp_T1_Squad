...
Estrutura esperada da pasta:
    ...
    ├── pagina_*.py
    └── data/
        ├── susep_real.xlsx       <- base de apolices
        ├── telemetria_esp32.csv  <- Anthony coloca aqui
        ├── frota_real.csv        <- cadastro de frota (opcional)
        └── modelo_xgboost.pkl    <- modelo treinado (opcional)
...
def painel_status_geral() -> None:
    ...
    for chave, info in ARQUIVOS_ESPERADOS.items():
        ...
        cor = "#3FCF8E" if existe else "#F5B23D"
            ...
            <div>
                <div style="display:flex;gap:8px;align-items:center;
                    ...
                    <span style="font-size:0.65rem;color:{cor};
                                 background:{cor}18;padding:2px 7px;
                                 border-radius:4px;font-weight:600;">
                        {'conectado' if existe else 'aguardando'}</span>
                </div>

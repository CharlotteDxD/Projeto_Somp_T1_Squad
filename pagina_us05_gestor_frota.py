...
def _frota_mock_us05():
    """
    Frota com dados completos para o Gestor.
    Mantem compatibilidade com o ranking original do Gustavo.
    """
    np.random.seed(42)
...
def _categoria_por_modo(score, modo):
    ...
    if score >= th["score_critico"]:
        ...
        return "ALERTA", "#F5A623"
    else:
        return "NORMAL", "#22D48A"
...
def _recomendacao_manutencao(row):
    ...
    if score >= 75 or ultima > 180:
        return ("URGENTE", "Inspecao completa em 24h", "#FF4D6A")
    elif score >= 50 or ultima > 120 or horas > 6000:
        return ("PROGRAMADA", f"Manutencao em ate 7 dias", "#F5A623")
    elif idade > 10:
        return ("PREVENTIVA", "Revisao trimestral recomendada", "#5B7FFF")
    else:
        return ("OK", "Operacao normal", "#22D48A")
...
def render():
    ...
    _header(
        "Gestor de Frota",
        "Gustavo",
        "US05 · Ranking · Alertas por Modo · Manutencao Preventiva · Cloud EC2"
    )
    ...
    <div style="background:{cor_api}10;border:1px solid {cor_api}40;
        ...
        <div style="font-size:0.85rem;color:#EEF0F8;">
            <strong>API Cloud (Gustavo):</strong>
            <span style="color:{cor_api};font-family:monospace;">{text_api}</span>
        </div>
        <div style="font-size:0.7rem;color:#8B8FA8;font-family:monospace;">
            {EC2_API_HOST}/health
        </div>
    </div>
        ...
        for idx, (_, row) in enumerate(df_filtrado.head(10).iterrows(), start=1):
            ...
            <div style="background:#0D0F18;border:1px solid {cor}33;
                ...
                <div style="display:flex;gap:14px;align-items:center;">
                    <span style="font-size:1.2rem;font-weight:800;color:#4A4D62;
                                 min-width:30px;font-family:monospace;">#{idx}</span>
                    <div>
                        <div style="font-weight:700;color:#EEF0F8;font-size:0.95rem;">
                            {row['Equipamento']} <span style="color:#8B8FA8;font-weight:400;
                                                              font-size:0.82rem;">
                                · {row['Tipo']}</span></div>
                        <div style="font-size:0.72rem;color:#8B8FA8;margin-top:2px;">
    ...
    with tab2:
        ...
        cores_modo  = {"Campo": "#22D48A", "Transporte": "#5B7FFF", "Misto": "#A855F7"}
        for col, (modo, th) in zip(col_modos, THRESHOLDS.items()):
            ...
            <div style="background:#0D0F18;border:1px solid {cor}33;
                ...
                <div style="font-size:0.75rem;color:#EEF0F8;line-height:1.9;">
                    ...
                    <div style="margin-top:8px;padding-top:8px;
                        ...
                        Score critico: <strong style="color:#FF4D6A;">≥ {th['score_critico']}</strong></div>
                    <div>Score alerta: <strong style="color:#F5A623;">≥ {th['score_alerta']}</strong></div>
                </div>
                <div style="margin-top:12px;padding-top:8px;
                            border-top:1px solid rgba(255,255,255,0.07);
                            font-size:0.72rem;color:#4A4D62;">
        ...
        if len(df_modo) > 0:
            ...
            st.markdown(f"""
            <div style="background:rgba(91,127,255,0.06);border:1px solid rgba(91,127,255,0.25);
                        border-left:3px solid #5B7FFF;border-radius:10px;
                        padding:12px 18px;margin-top:10px;font-size:0.85rem;color:#EEF0F8;">
                Com esses thresholds, <strong style="color:#F5A623;">{n_alertados}</strong>
        ...
        if urgentes:
            ...
            for row in urgentes:
                ...
                <div style="background:#0D0F18;border:1px solid {cor}33;
                            ...
                            display:flex;justify-content:space-between;align-items:center;">
                    <div>
                        <div style="font-weight:700;color:#EEF0F8;font-size:0.92rem;">
                            {row['Equipamento']} · {row['Tipo']} ({row['Regiao']})</div>
                        <div style="font-size:0.75rem;color:{cor};margin-top:3px;">
                            {msg}
                        </div>
                        <div style="font-size:0.7rem;color:#8B8FA8;margin-top:3px;">
    ...
    with tab4:
            ...
            for metodo, rota, desc, resp in endpoints:
                ...
                <div style="background:#131520;border:1px solid rgba(255,255,255,0.07);
                    ...
                    <div style="display:flex;gap:10px;align-items:center;
                        ...
                        <code style="font-size:0.82rem;color:#EEF0F8;">{rota}</code>
                    </div>
                    <div style="font-size:0.72rem;color:#8B8FA8;margin-left:36px;">
                        {desc} · <span style="color:#22D48A;">{resp}</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with col_api2:
            ...
            stack = [
                ("AWS EC2",       "t3.micro · Ubuntu Server",  "#FF4D6A"),
                ("FastAPI",       "uvicorn em :8000",          "#22D48A"),
                ("Modelo IA",     "LSTM + XGBoost (mock)",     "#5B7FFF"),
                ("SageMaker",     "Mock para producao futura", "#A855F7"),
                ("Streamlit",     "Dashboard original",        "#F5A623"),
            ]
    ...
    with tab5:
        ...
        if PLOTLY_OK:
            ...
            fig3.update_layout(plot_bgcolor="#0D0F18", paper_bgcolor="#0D0F18",
                               font=dict(color="#EEF0F8"), height=350)
    ...
    st.markdown("---")
    st.markdown(f"""
    <div style="background:rgba(255,77,106,0.04);border:1px solid rgba(255,77,106,0.2);
                border-left:3px solid #FF4D6A;border-radius:10px;
                padding:12px 18px;font-size:0.78rem;color:#8B8FA8;line-height:1.7;">
        <strong style="color:#EEF0F8;">Status de integracao US05 (Gustavo):</strong><br>
        <span style="color:#22D48A;">✓</span> Pagina criada e funcional ·
        <span style="color:#22D48A;">✓</span> Ranking original (Gustavo) preservado e enriquecido ·
        <span style="color:#22D48A;">✓</span> Alertas configuraveis por modo de uso ·
        <span style="color:#22D48A;">✓</span> Plano de manutencao preventiva ·
        <span style="color:#22D48A;">✓</span> Trilha de auditoria SHA-256 integrada ·
        <span style="color:#F5A623;">~</span> Aguardando IP publico da EC2 (atualmente fallback)
    </div>
    """, unsafe_allow_html=True)

...
def _categoria_score(score):
    if score >= 75: return "CRITICO",  "#FF4D6A"
    if score >= 50: return "ALTO",     "#F5A623"
...
def render():
    ...
    _header(
        ...
        "Rafael",
        "Sprint 1 · Arvore de Decisao + Isolation Forest + SHAP · MLP + LSTM"
    )
    ...
    with col_doc1:
        ...
        <div style="background:rgba(91,127,255,0.06);
            ...
            <div style="font-size:0.65rem;color:#5B7FFF;text-transform:uppercase;
                        letter-spacing:0.1em;margin-bottom:4px;">
                PDF Entregue — Rafael</div>
            <div style="font-size:0.9rem;color:#EEF0F8;font-weight:600;
                        margin-bottom:4px;">Machine Learning & Modelling</div>
            <div style="font-size:0.75rem;color:#8B8FA8;line-height:1.5;">
    ...
    with col_doc2:
        ...
        <div style="background:rgba(168,85,247,0.06);
                    ...
                    border-radius:10px;padding:12px 16px;">
            <div style="font-size:0.65rem;color:#A855F7;text-transform:uppercase;
                        letter-spacing:0.1em;margin-bottom:4px;">
                PDF Entregue — Rafael</div>
            <div style="font-size:0.9rem;color:#EEF0F8;font-weight:600;
    ...
    with tab1:
        ...
        with col_ft1:
            for feat, (tipo, grupo, motivo) in FEATURES_TABULARES.items():
                cor = CORES_GRUPO.get(grupo, "#8B8FA8")
                st.markdown(f"""
                <div style="background:#0D0F18;
                    ...
                    <div style="display:flex;gap:8px;align-items:center;
                        ...
                        <span style="font-size:0.65rem;color:#4A4D62;
                                     background:#131520;padding:2px 8px;
        ...
        with col_ft2:
            ...
            for g, n in grupos.items():
                ...
                <div style="background:#131520;border:1px solid {cor}33;
                    ...
                    <div style="font-size:0.7rem;color:#8B8FA8;">{g}</div>
                </div>
                """, unsafe_allow_html=True)
        ...
        origens = [
            ("SUSEP",       "Base oficial — distribuicoes e correlacoes reais de sinistros agricolas",  "#22D48A", "Guilherme"),
            ("Open-Meteo",  "API gratuita — pluviosidade e temperatura historica por GPS",              "#5B7FFF", "Todas"),
            ("ESP32/Wokwi", "Telemetria simulada — vibração, temperatura e contexto operacional em CSV","#A855F7", "Anthony"),
            ("Sintetico",   "Consolidacao dos tres acima, calibrada pela SUSEP, sem dados sensiveis",   "#F5A623", "Grupo T1"),
        ]
        col_o = st.columns(4)
    ...
    with tab2:
        ...
        st.markdown(f"""
        <div style="background:rgba(255,77,106,0.05);
                    border:1px solid rgba(255,77,106,0.2);
                    border-left:3px solid #FF4D6A;
                    border-radius:10px;padding:12px 18px;
                    margin:1rem 0;font-size:0.84rem;color:#8B8FA8;">
            <strong style="color:#EEF0F8;">Por que Recall e a metrica mais critica?</strong>
        ...
        with col_res1:
            ...
            <div style="background:{cor}12;border:1px solid {cor}44;
                ...
                <div style="font-size:3rem;font-weight:800;color:{cor};">
                    {score:.1f}</div>
                <div style="font-size:0.75rem;color:#8B8FA8;">/ 100</div>
    ...
    with tab3:
        ...
        if PLOTLY_OK:
            ...
            fig.update_layout(plot_bgcolor="#0D0F18", paper_bgcolor="#0D0F18",
                              font=dict(color="#EEF0F8"), height=420)
            st.plotly_chart(fig, use_container_width=True)
    ...
    with tab4:
        ...
        with col_shap1:
            ...
            <div style="background:{cor_sh}12;border:1px solid {cor_sh}44;
                        ...
                        margin-top:1rem;">
                <div style="font-size:0.7rem;color:#8B8FA8;">Score</div>
        ...
        with col_shap2:
            ...
            for feat, val in shap_vals.items():
                cor_b = "#FF4D6A" if val > 0 else "#22D48A"
                dir_  = "▲" if val > 0 else "▼"
                ...
                <div style="margin-bottom:10px;">
                    <div style="display:flex;justify-content:space-between;
                        ...
                        <span style="color:#EEF0F8;">{feat}</span>
                        <span style="color:{cor_b};font-weight:700;">
                            {dir_} {abs(val):.2f}</span>
                    </div>
                    <div style="background:#07080C;height:7px;border-radius:4px;">
            ...
            st.markdown(f"""
            <div style="background:rgba(91,127,255,0.06);
                        border-left:2px solid #5B7FFF;border-radius:6px;
                        padding:10px 14px;margin-top:1rem;
                        font-size:0.8rem;color:#EEF0F8;">
    ...
    with tab5:
        ...
        with col_mlp:
            st.markdown(f"""
            <div style="background:#0D0F18;border:1px solid #5B7FFF33;
                        border-top:2px solid #5B7FFF;border-radius:10px;
                        padding:1rem;text-align:center;height:100%;">
                <div style="font-size:0.7rem;color:#5B7FFF;
                            text-transform:uppercase;letter-spacing:0.1em;
                            margin-bottom:10px;">Ramo 1 — MLP</div>
                <div style="font-size:0.72rem;color:#8B8FA8;margin-bottom:8px;">
                    Dados tabulares do equipamento</div>
            """, unsafe_allow_html=True)
            for camada, desc in arq["ramo_mlp"]:
                st.markdown(f"""
                <div style="background:#131520;border-radius:6px;
                            padding:8px 12px;margin-bottom:4px;
                            font-size:0.75rem;">
                    <code style="color:#5B7FFF;">{camada}</code><br>
                    <span style="color:#8B8FA8;font-size:0.68rem;">{desc}</span>
                </div>
                """, unsafe_allow_html=True)
        ...
        with col_lstm:
            st.markdown(f"""
            <div style="background:#0D0F18;border:1px solid #A855F733;
                        border-top:2px solid #A855F7;border-radius:10px;
                        padding:1rem;text-align:center;height:100%;">
                <div style="font-size:0.7rem;color:#A855F7;
                            text-transform:uppercase;letter-spacing:0.1em;
            ...
            for camada, desc in arq["ramo_lstm"]:
                ...
                <div style="background:#131520;border-radius:6px;
                            padding:8px 12px;margin-bottom:4px;
                            font-size:0.75rem;">
                    <code style="color:#A855F7;">{camada}</code><br>
                    <span style="color:#8B8FA8;font-size:0.68rem;">{desc}</span>
                </div>
                """, unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        with col_fusao:
            st.markdown(f"""
            <div style="background:#0D0F18;border:1px solid #22D48A33;
                        border-top:2px solid #22D48A;border-radius:10px;
                        padding:1rem;text-align:center;height:100%;">
                <div style="font-size:0.7rem;color:#22D48A;
                            text-transform:uppercase;letter-spacing:0.1em;
            ...
            for camada, desc in arq["fusao"]:
                ...
                <div style="background:#131520;border-radius:6px;
                            padding:8px 12px;margin-bottom:4px;
                            font-size:0.75rem;">
                    <code style="color:#22D48A;">{camada}</code><br>
                    <span style="color:#8B8FA8;font-size:0.68rem;">{desc}</span>
                </div>
                """, unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        if PLOTLY_OK:
            ...
            with col_l:
                ...
                fig_loss.add_trace(go.Scatter(
                    ...
                    name="Train Loss", line=dict(color="#5B7FFF", width=2),
                ))
                fig_loss.add_trace(go.Scatter(
                    x=df_train["Epoch"], y=df_train["Val Loss"],
                    name="Val Loss",   line=dict(color="#FF4D6A", width=2),
                ))
            ...
            with col_a:
                ...
                fig_acc.add_trace(go.Scatter(
                    ...
                    name="Train Acc", line=dict(color="#22D48A", width=2),
                ))
                fig_acc.add_trace(go.Scatter(
                    x=df_train["Epoch"], y=df_train["Val Acc"],
                    name="Val Acc",   line=dict(color="#F5A623", width=2),
                ))
        ...
        for sprint, desc, cor, done in roadmap:
            ...
            <div style="background:#0D0F18;border:1px solid {cor}33;
                ...
                <div>
                    ...
                    <div style="font-size:0.78rem;color:#8B8FA8;">{desc}</div>
                </div>
    ...
    with tab6:
        ...
        comparativo = [
            ("Recall (sinistro)",  DT_METRICAS["recall_sinistro"],
             LSTM_CACHE["metricas"]["recall"],    "#FF4D6A"),
            ("F1-Score",           DT_METRICAS["f1_sinistro"],
             LSTM_CACHE["metricas"]["f1"],        "#F5A623"),
            ("ROC-AUC",            DT_METRICAS["roc_auc"],
             LSTM_CACHE["metricas"]["roc_auc"],   "#5B7FFF"),
            ("Precision",          DT_METRICAS["precision_sinistro"],
             LSTM_CACHE["metricas"]["precision"], "#22D48A"),
            ("Acuracia",           DT_METRICAS["acuracia"],
             LSTM_CACHE["metricas"]["acuracia"],  "#8B8FA8"),
        ]
        ...
        if PLOTLY_OK:
            ...
            fig.add_trace(go.Bar(
                ...
                name="Decision Tree",
                marker_color="#5B7FFF", opacity=0.8,
            ))
            fig.add_trace(go.Bar(
                x=metricas_nomes, y=vals_lstm,
                name="MLP + LSTM",
                marker_color="#22D48A", opacity=0.8,
            ))
            fig.update_layout(
                ...
                template="plotly_dark",
                plot_bgcolor="#0D0F18", paper_bgcolor="#0D0F18",
                font=dict(color="#EEF0F8"), height=380,
    ...
    <div style="background:rgba(91,127,255,0.04);
        ...
        <strong style="color:#EEF0F8;">Status — ML & Deep Learning:</strong><br>
        <span style="color:#22D48A;">✓</span>
